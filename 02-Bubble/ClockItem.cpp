...
namespace
{
	...
	const int CLOCK_ITEM_COL = 2;
	const int CLOCK_ITEM_ROW = 0;
}
...
void ClockItem::init(const glm::ivec2 &tilePos, ShaderProgram &shaderProgram)
{
	...
	const float uvY0 = float(CLOCK_ITEM_ROW) * tileH;
...
void ClockItem::render() const
{
	if(shaderProgram == NULL || collected)
		return;

	const float tileW = 1.f / float(ITEMS_SHEET_COLS);
	const float texDisplX = float(CLOCK_ITEM_COL) * tileW;
...
void ClockItem::collect()
{
	collected = true;
}

bool ClockItem::isCollected() const
{
	return collected;
}

glm::vec2 ClockItem::getPosition() const
{
	return worldPos;
}

glm::ivec2 ClockItem::getSize() const
{
	return size;
}
